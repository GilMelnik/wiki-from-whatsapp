"""Stage C.5: plan wiki pages, merges, and cross-links from aggregated claims.
...
PLAN_SYSTEM = (
    ...
    "הצע קישורים סמנטיים בין עמודים (עד 8 קישורים יוצאים לכל עמוד). "
...
def run(
    ...
    *,
    ...
    try:
        ...
        if not raw:
            from wiki_build.llm_client import extract_json
...
if __name__ == "__main__":
    meta = run(
        llm=LLMClient.for_stage("plan", use_hybrid_defaults=True),
        use_batch=True,
    )
    print(
        f"Planned {meta['page_count']} pages ({meta['link_count']} links) "
        f"-> {meta['output_path']}"
    )
