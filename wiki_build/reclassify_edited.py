...
from wiki_build.classify import (
    CLASSIFY_SYSTEM,
...
from wiki_build.threads_io import load_threads, render_thread_for_llm
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Re-run LLM classification on edited threads (skip manual rejects)."
    )
    parser.add_argument(
        "--threads",
        type=Path,
        default=EDITED_THREADS_PATH,
        help=f"Edited threads JSON (default: {EDITED_THREADS_PATH})",
    )
    parser.add_argument(
        "--classified",
        type=Path,
        default=EDITED_CLASSIFIED_PATH,
        help=f"Edited classifications JSON (default: {EDITED_CLASSIFIED_PATH})",
    )
    parser.add_argument(
        "--max-threads",
        type=int,
        default=None,
        help="Limit how many threads are sent to the LLM (pilot runs)",
    )
    parser.add_argument(
        "--no-batch",
        action="store_true",
        help="Use synchronous API even when batch is supported",
    )
    ...
    print(
        f"Reclassified {meta['reclassified_by_llm']} threads; "
        f"skipped {meta['skipped_not_knowledge_bearing']} not-knowledge-bearing; "
        f"{meta['knowledge_bearing_count']} knowledge-bearing total"
    )
